...
The Chronous Angular playground. The left rail carries the props — every field of
...
Angular directives from `@midstem/chronous-angular`. Under it a status strip prints
the summary of the `CalendarLayout`, and opens onto its raw JSON.

**Code** is that same board as one standalone Angular component file. It carries the
range, the events and the row height on screen, and the same Tailwind classes, the
same geometry and the same directives the board runs — including the scroll to 07:00
and the current-time line.

**Simple** is the short way to the same picture: no toolbar, no navigation, no
colour palette, no helper functions — the range, the events, and plain directives.

The board is styled with Tailwind CSS v4 and shares logic with the React playground
...
```bash
npm run dev --workspace @midstem/chronous-playground-angular
```

...
export const styleOf = (
  ...
  style && typeof style === 'object'
    ? { ...layout, ...(style as Record<string, unknown>) }
    : layout

export type SlotFn<TScope = any> = (scope: TScope) => any

export const renderSlot = <TScope>(
  slot: SlotFn<TScope> | undefined,
  scope: TScope,
  fallback?: any
): any => (slot ? slot(scope) : (fallback ?? undefined))
